...
- дополнительная диагностика обычным двухфазным симплекс-методом для случаев, когда стартовая таблица не является двойственно допустимой;
...
| `dual_simplex.py` | Алгоритм двойственного симплекс-метода и дополнительная двухфазная диагностика |
...
## Особенность варианта 17

Для варианта 17 начальная таблица, построенная напрямую из модели, не является двойственно допустимой. Поэтому программа фиксирует этот факт, затем автоматически запускает дополнительную двухфазную диагностику и получает математический статус задачи: целевая функция не ограничена сверху на множестве допустимых решений.
