...
from linear_problem import Constraint, LinearProblem, LpResult
...
class SimplexSolver:
    def __init__(
        ...
        self.original_variable_names = problem.variable_names[:]
    ...
    def solve(self) -> LpResult:
        ...
        if len(artificial_columns) > 0:
            ...
            self._snapshot(
                title="Фаза I. Исходная симплекс-таблица LP-релаксации",
                phase="I",
                iteration=0,
                tableau=tableau,
                ...
                basis=basis,
                objective_label="W",
                notes="Введены искусственные переменные. Вспомогательная функция максимизирует -сумму искусственных переменных.",
            )
            ...
            if phase_one_status == "unbounded":
                return LpResult(
                    ...
                    message="В фазе I не удалось получить допустимое опорное решение.",
                )
            ...
            if tableau[-1][-1] != 0:
                return LpResult(
                    ...
                    variables={name: Fraction(0) for name in self.original_variable_names},
                    iterations=self.iterations,
                    message="LP-релаксация несовместна: максимум вспомогательной функции W не равен 0.",
                )

            self._snapshot(
                title="Фаза I завершена для LP-релаксации",
                phase="I",
                ...
                basis=basis,
                objective_label="W",
                notes="Получено допустимое опорное решение. Искусственные переменные подлежат удалению.",
            )
            ...
            self._snapshot(
                title="Переход LP-релаксации к фазе II",
                phase="II",
                iteration=0,
                tableau=tableau,
                column_names=column_names,
                basis=basis,
                objective_label="F",
                notes="Искусственные переменные удалены. В таблицу подставлена исходная целевая функция LP-релаксации.",
            )
        else:
            tableau.append(self._build_objective_row(phase_two_costs, tableau, basis))

            self._snapshot(
                title="Фаза II. Исходная симплекс-таблица LP-релаксации",
                phase="II",
                iteration=0,
                tableau=tableau,
                column_names=column_names,
                basis=basis,
                objective_label="F",
                notes="Искусственные переменные не требуются: начальный базис получен с помощью остаточных переменных.",
            )
        ...
        if phase_two_status == "unbounded":
            return LpResult(
                ...
                variables={name: Fraction(0) for name in self.original_variable_names},
                iterations=self.iterations,
                message="LP-релаксация неограничена.",
            )
        ...
        self._snapshot(
            title="Итоговая симплекс-таблица LP-релаксации",
            phase="II",
            iteration=self.iterations,
            tableau=tableau,
            column_names=column_names,
            basis=basis,
            objective_label="F",
            notes="Оптимальное решение LP-релаксации найдено.",
        )
        ...
        return LpResult(
            ...
            variables=variables,
            iterations=self.iterations,
            message="Оптимальное решение LP-релаксации найдено.",
        )
    ...
    def _normalize_constraints(self, constraints: List[Constraint]) -> List[Constraint]:
        ...
        for constraint in constraints:
            ...
            normalized_constraints.append(
                Constraint(coefficients=coefficients, sign=sign, rhs=rhs, name=constraint.name)
            )
    ...
    def _run_simplex(
        ...
        while True:
            ...
            if entering_column is None:
                self._snapshot(
                    title=f"Фаза {phase}. Условие оптимальности LP-релаксации выполнено",
                    phase=phase,
            ...
            if leaving_row is None:
                self._snapshot(
                    title=f"Фаза {phase}. LP-релаксация неограничена",
                    phase=phase,
                    iteration=local_iteration,
                    tableau=tableau,
                    column_names=column_names,
            ...
            self._snapshot(
                title=f"Фаза {phase}. Итерация {local_iteration} LP-релаксации",
                phase=phase,
                iteration=local_iteration,
                tableau=tableau,
                column_names=column_names,
    ...
    def _remove_artificial_variables_from_basis(
        ...
        for row_index, basis_column in enumerate(basis):
            ...
            self._snapshot(
                title="Искусственная переменная LP-релаксации выведена из базиса",
                phase="I",
                iteration=self.iterations,
                tableau=tableau,
                column_names=column_names,
