  ...
  "stages": [
    {
      ...
      "states": ["S0"],
      "transitions": [
        {"state": "S0", "decision": "a", "next_state": "S1", "value": 5}
      ]
    },
    {
      "name": "2",
      "states": ["S1"],
      "transitions": [
        {"state": "S1", "decision": "b", "next_state": "S2", "value": 3}
      ]
    }
  ]
}
...
Если `n` или `c` заданы символически или отсутствуют, программа для исходного направления `max` возвращает статус `symbolic_solution` и формирует общий аналитический вывод по случаям: при `n = 1`, `c >= 0` решение единственно и `z = c^2`; при `n >= 2`, `c >= 0` задача не ограничена сверху; при `c < 0` допустимых решений нет. Если заданы численные `n >= 2` и `c >= 0`, программа возвращает статус `unbounded`.
...
| `input_required` | нужны конкретные значения параметров для выбранного режима |
| `symbolic_solution` | получено общее аналитическое решение при символических параметрах |
