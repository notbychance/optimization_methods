...
from dp_models import FiniteHorizonProblem, KnapsackItem, KnapsackProblem, Stage, Transition
...
class ProblemReader:
    @staticmethod
    def read(file_path: str) -> Union[KnapsackProblem, FiniteHorizonProblem]:
        path = Path(file_path)
        if not path.exists():
        ...
        raise ValueError("Поле problem_type должно быть равно 'knapsack' или 'finite_horizon'.")
